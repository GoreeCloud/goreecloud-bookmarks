...
For version 0.1.1, source validation is present, but the owning Bookmarks `main` branch does not yet expose the documented extension-capture HTTP endpoint. The extension remains an `authenticated-source-candidate`; application-side endpoint acceptance, real Firefox runtime, and signed-artifact gates remain mandatory.
