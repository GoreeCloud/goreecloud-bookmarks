...
func (c RetrievalClient) fetchBookmarksPage(ctx context.Context, token, after string) (retrievalResponse, error) {
	...
	if result.Dataset != "bookmarks.items" || result.Count != len(result.Records) || len(result.Records) > retrievalPageSize {
		return retrievalResponse{}, ErrSyncRetrievalFailed
	}
	previousID := after
	for _, record := range result.Records {
		if record.Dataset != "bookmarks.items" || record.SchemaVersion < 1 || record.RecordID == "" || record.Revision == 0 || record.UpdatedAt.IsZero() || strings.TrimSpace(record.OriginDevice) == "" {
			return retrievalResponse{}, ErrSyncRetrievalFailed
		}
