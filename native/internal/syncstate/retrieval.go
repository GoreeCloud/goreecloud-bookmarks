...
var ErrSyncRetrievalFailed = errors.New("bookmark sync retrieval failed")
...
func (c RetrievalClient) FetchBookmarks(ctx context.Context) ([]Envelope, error) {
	...
	for page := 0; page < maxRetrievalPages; page++ {
		result, err := c.fetchBookmarksPage(ctx, token, after)
		if err != nil {
			return nil, err
		}
...
func (c RetrievalClient) fetchBookmarksPage(ctx context.Context, token, after string) (retrievalResponse, error) {
	if len(after) > maxSyncRecordIDBytes {
		return retrievalResponse{}, ErrSyncRetrievalFailed
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, strings.TrimRight(c.BaseURL, "/")+"/api/v1/sync/bookmarks/items", nil)
	if err != nil {
	...
	if result.Dataset != "bookmarks.items" || result.Count != len(result.Records) || len(result.Records) > retrievalPageSize || len(result.NextAfter) > maxSyncRecordIDBytes {
		return retrievalResponse{}, ErrSyncRetrievalFailed
	}
	previousID := after
	for _, record := range result.Records {
		if record.Dataset != "bookmarks.items" || record.SchemaVersion < 1 || record.RecordID == "" || len(record.RecordID) > maxSyncRecordIDBytes || record.Revision == 0 || record.UpdatedAt.IsZero() || strings.TrimSpace(record.OriginDevice) == "" {
			return retrievalResponse{}, ErrSyncRetrievalFailed
		}
