...
func (c RetrievalClient) fetchBookmarksPage(ctx context.Context, token, after string) (retrievalResponse, error) {
	...
	if result.Dataset != bookmarksItemsDataset || result.Count != len(result.Records) || len(result.Records) > retrievalPageSize || len(result.NextAfter) > maxSyncRecordIDBytes {
		return retrievalResponse{}, ErrSyncRetrievalFailed
	}
	previousID := after
	for _, record := range result.Records {
		if !validBookmarkEnvelope(record) || (previousID != "" && record.RecordID <= previousID) {
			return retrievalResponse{}, ErrSyncRetrievalFailed
		}
